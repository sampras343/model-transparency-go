...
func TestNewPkcs11Signer_MissingModelPath(t *testing.T) {
	...
	opts := Pkcs11SignerOptions{
		...
		Pkcs11URI:     "pkcs11:token=test;object=test",
	}
...
func TestNewPkcs11Signer_MissingPkcs11URI(t *testing.T) {
	tmpDir := t.TempDir()

	opts := Pkcs11SignerOptions{
		ModelPath:     tmpDir,
		SignaturePath: filepath.Join(tmpDir, "sig.json"),
		Pkcs11URI:     "", // Missing URI
	}
...
func TestNewPkcs11Signer_ValidPaths(t *testing.T) {
	...
	if signer.opts.Pkcs11URI != opts.Pkcs11URI {
		t.Errorf("Expected PKCS#11 URI %s, got %s", opts.Pkcs11URI, signer.opts.Pkcs11URI)
	}
}
